...
class Solution:
    def numberOfArithmeticSlices(self, nums: List[int]) -> int:
        total = 0 # Initialize the total number of arithmetic slices
        ...
        for i in range(len(nums)): # Iterate through the list of numbers (nums)
            for j in range(i):  # Iterat over all element sbefore the i-th elemtn using index j
                ...
                total += prev_count # Add only the count of subseqeunces that are lready at least 2 elements long
